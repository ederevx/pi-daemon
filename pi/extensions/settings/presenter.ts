...
export class DaemonSettingsPresenter {
	...
	specs(): SettingSpec[] {
		...
		return [
			this.number("idleReapSeconds", "Idle reap",
				"End detached model-idle sessions after this; 0 disables",
				"PI_PTYD_IDLE_REAP", 21600),
			this.number("idleWarnGraceSeconds", "Idle warning grace",
				"Warning window before the reap; delete the file to stay alive",
				"PI_PTYD_IDLE_WARN_GRACE", 300, "PI_PTYD_FINISH_GRACE"),
			this.number("daemonIdleTimeoutSeconds", "Daemon idle timeout",
				"Self-shutdown when nothing is attached or running; 0 disables",
				"PI_DAEMON_IDLE_TIMEOUT", 300),
			...
			this.number("ticketTtlSeconds", "Ticket TTL",
				"Finished tickets expire after this",
				"PI_PTYD_TICKET_TTL", 86400),
